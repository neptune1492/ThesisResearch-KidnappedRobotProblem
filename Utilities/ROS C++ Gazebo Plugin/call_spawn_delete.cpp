...
//	std::cout<<_world->GetName();
	...
	//ros::SubscribeOptions subOpts = ros::SubscribeOptions::create<tf2_msgs::TFMessage>("/tf", 1, boost::bind(&CallSpawnDeleteWorldPlugin::OnMsgTest, this, _1), ros::VoidPtr(), &this->callbackQueue);
...
//	this->timer = this->rosNode->createTimer(ros::Duration(1), boost::bind(&CallSpawnDeleteWorldPlugin::timerCallback, this, _1));

//	this->timer = this->rosNode->createTimer(ros::Duration(70), boost::bind(&CallSpawnDeleteWorldPlugin::timerCallbackMinor, this, _1));//for orientation


	this->timer = this->rosNode->createTimer(ros::Duration(15), boost::bind(&CallSpawnDeleteWorldPlugin::timerCallbackMajor, this, _1)); //45 (Major 1-10? 11?) and 65.
...
//	this->joySub = this->rosNode->subscribe(subOpts);
	...
	//commandSubscriber = node->Subscribe("~/my_topic",&CallSpawnDeleteWorldPlugin::callbackFunction, this);
...
//********BELOW IS FROM TRYING TO USE ROS MESSAGES.******
	...
	ros::SubscribeOptions so = 
		ros::SubscribeOptions::create<std_msgs::String>("/"+_world->GetName() + "/vel_cmd",
		1,
...
callback_time = ros::Time::now();
	...
	ros::Time lower_seconds(30.0);
	ros::Time upper_seconds(40.0);
...
void timerCallbackMinor(const ros::TimerEvent& event){
	...
	x_prime = current_x;
	y_prime = current_y;
	theta_prime = current_theta + 0.17;//10deg = 0.17 rad
...
void timerCallbackMajor(const ros::TimerEvent& event){
	...
	x_prime = current_x + 0.2;
	y_prime = current_y+0.2;
...
std::vector<double> calculate_new_pose(double theta0, double x0, double y0, double theta1, double x1, double y1, double theta2, double x2, double y2){
		...
		if((std::abs(theta0 - theta1) < .00174) || (std::abs(theta2 - theta1) < .00174)) {// angles are close enough to assume straight line - 1.74e-6 rad <-> .1 deg
			...
			vnewX_newY_newTheta[0] = x2 + radius * cos(theta1+(180*(180/3.14)));
			vnewX_newY_newTheta[1] = y2 + radius * sin(theta1+(180*(180/3.14)));//Tryin to do angle of prev+180 to get across the circle
			...
			//myGazeboFile<<"\t"<<vintx_inty[0]<<"\t"<<vintx_inty[1];



			//std::vector<double> vnewX_newY_newThetaTest;
			vxc_yc_mainradius=draw_circles(theta1,x1, y1, theta2, x2, y2, intx, inty); //Orig method
		//for some reason, I was usin theta0 with x1,y1 and theta1 with x2 and y2...
		
	
			//NEW! METHOD! OF! GETTING! CIRLCES!
			//vxc_yc_mainradius=draw_circles(theta0, x0, y0, theta1,x1, y1, theta2, x2, y2);//3pt method



			//Draw Arcs
			//pass tehta0 with x1,y1; theta1 with x2, y2
			vnewX_newY_newTheta=draw_arcs(theta1, x1, y1, theta2, x2, y2, vxc_yc_mainradius[0], vxc_yc_mainradius[1],vxc_yc_mainradius[2]);	
	

	
			//		this->myGazeboFile<<"\t"<<vnewX_newY_newTheta[0]<<"\t"<<vnewX_newY_newTheta[1]<<"\t"<<vnewX_newY_newTheta[2];	

	
		
			//vnewX_newY_newThetaTest=draw_arcs(theta1, x1, y1, theta2, x2, y2, vxc_yc_mainradiusTest[0], vxc_yc_mainradiusTest[1],vxc_yc_mainradiusTest[2]);	


			//		this->myGazeboFile<<"\t"<<vnewX_newY_newThetaTest[0]<<"\t"<<vnewX_newY_newThetaTest[1]<<"\t"<<vnewX_newY_newThetaTest[2];	
			}

/*
			vnewX_newY_newTheta[0] = tx;
			vnewX_newY_newTheta[1] = ty;
			vnewX_newY_newTheta[2] = theta2;
*/
...
std::vector<double> draw_circles(double angleA, double posxA, double posyA, double angleB,double posxB, double posyB, double angleC, double posxC, double posyC){
	...
	else if (xDelta_b == 0)	{	//bslope == infiinty
		yc = BC_Mid_y;
...
std::vector<double> draw_arcs(double angle1, double posx1, double posy1, double angle2, double posx2, double posy2, double centX, double centY, double radius){
...
//% 10% change in one wheel speed results in 10% change in radius		//Are we sure it's a 10% change in raidus? or is it 5?

//%based on direction
//%negative dir means turning left
//%positive dir means turning right
new_rad = radius * 0.8;//Change radius here!
