...
%%Variables:
x_tw = joinedsetsturtlebotteleop1(:,1);

column = 3;
numColumns = 7;
inputs =[];
for c = column:numColumns
    inputs=[inputs table2array([joinedsetsturtlebotteleop1(:,c);joinedsetsturtlebotteleop5(:,c);])]; %joinedsetsturtlebotteleop3(:,c); joinedsetsturtlebotteleop4(:,c); joinedsetsturtlebotteleop5(:,c);])];
end
inputs = inputs';

amcl_gazebo_error = [ joinedsetsturtlebotteleop1(:,'AMCL_GazeboVolume'); joinedsetsturtlebotteleop5(:,'AMCL_GazeboVolume');];
...
inputs =[];
for c = column:numColumns
    new_inputs=[inputs table2array([joinedsetsturtlebotteleop5(:,c)]);];
end
